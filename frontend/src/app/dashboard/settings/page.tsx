...
export default function ProfilePage() {
    ...
    const handleUseGoogleAvatar = async () => {
        if (user?.googleId) {
            ...
            if (googleAvatarUrl && googleAvatarUrl.startsWith('http')) {
                ...
                setMessage({ type: 'error', text: 'Google avatar not synced. Click "Sync from Google" to refresh.' });
            }
    ...
    const handleSyncGoogleProfile = () => {
        const token = localStorage.getItem('auth_token');
        if (!token) return;
        window.location.href = `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000/api'}/auth/google/sync?token=${token}`;
    };

    useEffect(() => {
        const params = new URLSearchParams(window.location.search);
        if (params.get('synced') === 'true') {
            setMessage({ type: 'success', text: 'Profile synced with Google successfully!' });
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {user.googleId && (
                                        <Button 
                                            variant="outline" 
                                            size="sm" 
                                            className="h-8 border-slate-200 text-slate-600 hover:text-indigo-600 hover:bg-indigo-50"
